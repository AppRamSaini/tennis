...
import 'package:tennis/repository/reports.dart';
...
class ReportsProvider extends ChangeNotifier {
  ...
  void setValueLoserWinner(String winnerUUID,String winnerName,String challengUUID,String challengName,String acceptUUID,String acceptName){
    AppConfig.winnerUuid = winnerUUID;
    AppConfig.winnerName = winnerName;
    if(AppConfig.winnerUuid == acceptUUID){
      AppConfig.loserUuid = challengUUID;
    }else {
      AppConfig.loserUuid = acceptUUID;
    }
    if(AppConfig.winnerName == acceptName){
      AppConfig.loserName = challengName;
    }else {
      AppConfig.loserName = acceptName;
    }
    notifyListeners();
  }
  void leaguesScorePendingReports(BuildContext context) async {
    try {
      Helpers.verifyInternet().then((intenet) {
        if (intenet != null && intenet) {
          _isLoading = true;
          notifyListeners();
          getLeaguesScorePendingReports(context).then((response) {
            if (json.decode(response.body)['status'] == true) {
              reportsList = json.decode(response.body)['reports'];
              _isLoading = false;
              notifyListeners();
  ...
  void leaguesScoreReportUpdate(BuildContext context,String reportUuid,String reportAction,String winnerUuid,String sets,String status,String score) async {
    try {
      Helpers.verifyInternet().then((intenet) {
        if (intenet != null && intenet) {
          sendLeaguesScoreReportUpdate(context,reportUuid,reportAction,winnerUuid,sets,status,score).then((response) {
  ...
  void tieBreakBoolValueChange(int pos){
    if(pos == 0){
      fTB = true;
      fTBVCheck = true;
    }else if (pos == 1){
